...
function QuestionCard({ question, currentQuestion, totalQuestions, onAnswer, timeLeft }) {
  ...
  const useHint = () => {
    ...
    setFilteredAnswers([correctAnswer, randomWrongAnswer].sort(() => Math.random() - 0.5));
    setUsedHint(true);
  };

  const handleAnswerClick = index => {
    if (showResult) return; // Предотвращаем повторные клики
  ...
  return (
    ...
    >
          ...
          {(usedHint ? filteredAnswers : question.answers).map((answer, index) => {
            ...
            return (
              <motion.button
                ...
                onClick={() => handleAnswerClick(actualIndex)}
      ...
      >
        <motion.span
          className={`text-lg font-medium ${timeLeft <= 5 ? 'text-red-500' : 'text-[#707579]'}`}
          animate={
            timeLeft <= 5
              ? {
                  scale: [1, 1.1, 1],

...
function QuizApp() {
  ...
  const handleTimeout = useCallback(() => {
    ...
    playSound('timeout');
    const newAnswers = [...answers, { isCorrect: false, timeSpent: difficulty.timePerQuestion }];
    setAnswers(newAnswers);

    if (currentQuestion < difficulty.questionsCount - 1) {
      setCurrentQuestion(prev => prev + 1);
      setTimeLeft(difficulty.timePerQuestion);
    } else {
      setGameStarted(false);
    }
  }, [answers, currentQuestion, difficulty, questions, playSound]);
    ...
    try {
      ...
      const validData = data.every(
        q =>
          ...
          q.answers.every(a => typeof a.text === 'string' && typeof a.isCorrect === 'boolean')
      );
  ...
  const handleAnswer = selectedIndex => {
    ...
    // Проверяем, что индекс валидный и ответ существует
    const currentAnswers = questions[currentQuestion].answers;
    if (selectedIndex !== -1 && (!currentAnswers || !currentAnswers[selectedIndex])) {
      console.error('Некорректный индекс ответа');
      return;
    }

    const isCorrect = selectedIndex === -1 ? false : currentAnswers[selectedIndex].isCorrect;

    playSound(isCorrect ? 'correct' : 'incorrect');

    const timeSpent = difficulty.timePerQuestion - timeLeft;
    setAnswers(prev => [...prev, { selectedIndex, isCorrect, timeSpent }]);
    ...
    if (currentQuestion + 1 < questions.length) {
      ...
      WebApp.sendData(
        JSON.stringify({
          ...
          score: answers.filter(a => a.isCorrect).length + (isCorrect ? 1 : 0),
  ...
  }, [timeLeft, gameStarted, handleTimeout]);
